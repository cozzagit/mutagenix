...
export async function POST(
  ...
  // 9. Auto-create or update clan for both players
  // Find founder creature for requester
  const [requesterFounder] = await db
    .select()
    .from(creatures)
    .where(
      and(
  ...
  if (requesterFounder) {
    let [requesterClan] = await db
      .select()
      .from(clans)
      .where(eq(clans.founderId, requesterFounder.id));

    if (!requesterClan) {
      [requesterClan] = await db
        .insert(clans)
        .values({
          founderId: requesterFounder.id,
          ownerId: breedingRequest.requesterId,
          name: `Clan di ${requesterFounder.name}`,
          totalMembers: 1,
          totalGenerations: 1,
        })
        .returning();
      ...
      await db.insert(clanMemberships).values({
        clanId: requesterClan.id,
        creatureId: requesterFounder.id,
        userId: breedingRequest.requesterId,
        role: 'founder',
      });
    }

    // Add offspring A to requester's clan
    await db.insert(clanMemberships).values({
      clanId: requesterClan.id,
      creatureId: offspringA.id,
      userId: breedingRequest.requesterId,
      role: 'member',
    });
    ...
    await db
      ...
      .set({
        ...
        totalGenerations: sql`GREATEST(${clans.totalGenerations}, ${breedingResult.offspringA.familyGeneration})`,
        updatedAt: now,
      })
      .where(eq(clans.id, requesterClan.id));
  }

  // Find founder creature for target
  const [targetFounder] = await db
    .select()
    .from(creatures)
    .where(
      and(
  ...
  if (targetFounder) {
    let [targetClan] = await db
      .select()
      .from(clans)
      .where(eq(clans.founderId, targetFounder.id));
    ...
    if (!targetClan) {
      [targetClan] = await db
        ...
        .values({
          ...
          name: `Clan di ${targetFounder.name}`,
          totalMembers: 1,
          totalGenerations: 1,
        })
        .returning();
      ...
      await db.insert(clanMemberships).values({
        ...
        creatureId: targetFounder.id,
        userId: session.userId,
        role: 'founder',
      });
    }

    // Add offspring B to target's clan
    await db.insert(clanMemberships).values({
      clanId: targetClan.id,
      creatureId: offspringB.id,
      userId: session.userId,
      role: 'member',
    });
    ...
    await db
      ...
      .set({
        totalMembers: sql`${clans.totalMembers} + 1`,
        totalGenerations: sql`GREATEST(${clans.totalGenerations}, ${breedingResult.offspringB.familyGeneration})`,
        updatedAt: now,
      })
      .where(eq(clans.id, targetClan.id));
  }
